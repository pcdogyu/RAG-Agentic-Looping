...
- Python tooling (`backend/**`, Python files under `infra/**`, or `pyproject.toml`): run Ruff and the relevant pytest suite; use the full Python suite only for cross-cutting Python changes.
- Go-only changes do not require Ruff or pytest unless they modify a shared database/API contract that is still consumed by Python code.
