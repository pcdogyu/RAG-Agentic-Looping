...
- The canonical delivery branch is `main` on the `origin` remote (`pcdogyu/RAG-Agentic-Looping`).
- After every requested version change, run the relevant automated checks, commit the task-specific changes, and push the resulting commit to `origin/main` without waiting for a separate push request.
