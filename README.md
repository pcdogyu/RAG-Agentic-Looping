...
Web 看板和模型日志页底部展示构建提交时间、提交 ID 与分支。直接运行 `npm run build` 时 Vite 自动读取当前 Git 仓库；Docker 构建应显式传入同一份构建坐标：

```powershell
$env:VITE_BUILD_COMMIT_ID = git rev-parse HEAD
$env:VITE_BUILD_COMMIT_TIME = git show -s --format=%cI HEAD
$env:VITE_BUILD_BRANCH = git branch --show-current
docker compose build web
```

未传入 Docker 构建参数时页脚会显示 `unknown`，不会把宿主机 `.git` 目录复制进镜像。

### 当前测试覆盖

- 事件抽取、跨批次持久化事件簇、来源血缘折叠和中断恢复。
- 资产映射与统一标识。
- 时间边界与历史回放。
- FMP MCP/REST 传输与降级。
