...
func TestNativeOperationsMatchFrozenContractAndCompleteBatchThree(t *testing.T) {
	...
	if got, want := len(operations), 80; got != want {
...
func TestMigrationStatusIsDerivedFromRegisteredRoutes(t *testing.T) {
	...
	if payload.Total != 80 || payload.Native != 80 || payload.Remaining != 0 || !payload.Ready {
	...
	if payload.Worker.Runtime != "go" || payload.Worker.QueueBackend != "postgresql" || !payload.Worker.CutoverReady || len(payload.Worker.Order) != 11 {
