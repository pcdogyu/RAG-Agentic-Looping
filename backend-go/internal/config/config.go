...
type Config struct {
	...
	RecentResearchFilter    time.Duration
	...
	ResearchFastContext     int
	ResearchFastMaxOutput   int
	ResearchDeepConcurrency int
	ResearchThink           bool
...
func Load() (Config, error) {
	cfg := Config{
		...
		RecentResearchFilter:    time.Duration(envInt("RECENT_RESEARCH_FILTER_HOURS", 48)) * time.Hour,
		...
		ResearchFastContext:     envInt("OLLAMA_RESEARCH_FAST_CONTEXT_LENGTH", 16384),
	...
	if cfg.ResearchContextLength < 1 || cfg.ResearchMaxOutput < 1 || cfg.ResearchFallbackMax < 1 || cfg.ResearchFastContext < 1 || cfg.ResearchFastMaxOutput < 1 {
		return Config{}, fmt.Errorf("research context and output limits must be positive")
	}
	if cfg.WorkerLane == "research" && (cfg.ResearchDeepConcurrency < 1 || cfg.ResearchDeepConcurrency > cfg.WorkerConcurrency) {
		return Config{}, fmt.Errorf("OLLAMA_RESEARCH_DEEP_MAX_CONCURRENCY must be between 1 and GO_WORKER_CONCURRENCY")
	}
