...
func TestUpCreatesFreshGoRuntimeSchema(t *testing.T) {
	...
	for _, table := range []string{"fundamental_snapshots", "consensus_snapshots", "management_guidance_snapshots", "guidance_source_documents", "guidance_source_reviews", "forecast_versions", "event_assumption_links", "valuation_runs", "fundamental_rating_states", "fundamental_rating_revisions", "rating_invalidation_rules", "prediction_models", "probability_calibrations", "prediction_runs", "outcome_records", "shadow_prediction_comparisons", "model_governance_checks", "market_price_observations", "licensed_benchmark_price_import_receipts", "market_tradability_observations", "market_tradability_import_receipts", "fundamental_research_plans", "analyst_evidence_records", "fundamental_ai_batches", "fundamental_ai_runs", "fundamental_ai_source_snapshots", "fundamental_ai_candidates", "corporate_action_observations", "benchmark_mapping_observations", "security_universe_snapshots", "security_universe_memberships", "evaluation_holdout_reservations", "evaluation_dataset_versions", "evaluation_dataset_folds", "evaluation_dataset_samples", "evaluation_experiments", "evaluation_experiment_variants", "evaluation_experiment_predictions", "research_quality_reviews", "evaluation_performance_reports", "evaluation_final_holdout_reports", "model_failure_drills"} {
		var exists bool
		if err := pool.QueryRow(ctx, `SELECT to_regclass($1) IS NOT NULL`, schema+"."+table).Scan(&exists); err != nil || !exists {
...
var legacyORMColumns = map[string][]string{
	...
	"policy_impact_reviews":    {"policy_evaluation_id", "reviewer", "decision", "note", "created_at"},
...
var legacyORMPrimaryKeys = map[string][]string{
	...
	"policy_impact_reviews": {"policy_evaluation_id"},
...
var legacyORMIndexes = []string{
	...
	"ix_policy_impact_reviews_decision",
