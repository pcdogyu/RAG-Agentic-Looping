...
MAPPING_TASK = "market_loop.resolve_event_assets"
ARCHIVE_PREFIX = "market-loop:archive:mapping-cutover:"


@dataclass(frozen=True)
class LegacyMappingMessage:
    queue_key: bytes
    task_id: str
    event_id: str
    kwargs: dict[str, Any]
...
def decode_legacy_message(queue_key: bytes, raw: bytes) -> LegacyMappingMessage:
    envelope = json.loads(raw)
    headers = envelope.get("headers") or {}
    task_type = str(headers.get("task") or "")
    if task_type != MAPPING_TASK:
        raise ValueError(f"unsupported mapping task: {task_type or 'missing'}")
    ...
    if not isinstance(args, list) or not args:
        raise ValueError(f"mapping task {task_id} has no event id")
    event_id = str(args[0])
    UUID(event_id)
    if not isinstance(kwargs, dict):
        raise ValueError(f"mapping task {task_id} has invalid kwargs")
    properties = envelope.get("properties") or {}
    return LegacyMappingMessage(
        queue_key=queue_key,
        task_id=task_id,
        event_id=event_id,
        kwargs=kwargs,
...
def discover_legacy_messages(client: Redis) -> tuple[list[bytes], list[LegacyMappingMessage]]:
    keys = sorted(
        key for key in client.scan_iter(match="mapping*") if client.type(key) == b"list"
    )
    messages = [
    ...
    if len(task_ids) != len(set(task_ids)):
        raise ValueError("duplicate task ids found in legacy mapping queues")
...
def migrate_legacy_messages(client: Redis, *, apply: bool = False) -> dict[str, Any]:
    ...
    with SessionLocal() as db:
        known_events = set(
            db.execute(
                text(
                    "SELECT id::text FROM news_events "
                    "WHERE id::text = ANY(CAST(:ids AS text[]))"
                ),
                {"ids": list({item.event_id for item in messages})},
            ).scalars()
        )
        missing = sorted({item.event_id for item in messages} - known_events)
        if missing:
            raise RuntimeError(f"news_events rows missing: {len(missing)}")
        for item in messages:
            payload = json.dumps(
                {"args": [item.event_id], "kwargs": item.kwargs},
                ensure_ascii=False,
                separators=(",", ":"),
            )
            inserted = db.execute(
                text(
                    ...
                    INSERT INTO go_jobs(
                        ...
                        :id,'assist',:task_type,CAST(:payload AS jsonb),'queued',
                ...
                {
                    "id": item.task_id,
                    "task_type": MAPPING_TASK,
                    "payload": payload,
                    "priority": item.priority,
                    "dedupe_key": f"mapping:{item.event_id}",
                },
            ).scalar_one_or_none()
            if inserted is None:
                existing = db.execute(
                    text(
                        ...
                        WHERE id=:id OR (
                            queue='assist' AND dedupe_key=:dedupe_key
                            AND status IN ('queued','running','retrying')
                        )
                        LIMIT 1
                        """
                    ),
                    {"id": item.task_id, "dedupe_key": f"mapping:{item.event_id}"},
                ).scalar_one_or_none()
                if existing is None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Move drained legacy mapping messages into durable Go jobs."
    )
